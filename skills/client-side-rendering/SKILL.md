...
- `hooks-pattern` — CSR apps built with React commonly use hooks (`useState`, `useEffect`) to manage client-driven data fetching and rendering.
- `hoc-pattern` — Higher-Order Components are an alternative way to share cross-cutting logic (e.g. loading states, auth checks) across CSR views.
