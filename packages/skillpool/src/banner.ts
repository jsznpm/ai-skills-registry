...
const SEEN_MARKER = join(homedir(), ".skillpool", "seen");
...
export function showBannerOnce(): void {
  ...
  // stderr so piping `skillpool list | …` stays clean on the first run too.
  console.error(BANNER);
  try {
    mkdirSync(join(homedir(), ".skillpool"), { recursive: true });
